...
/*
 * guardar_op_banavih.php
 * Recibe POST con los datos del formulario nueva_op_banavih.php,
 * inserta ordenes_pago y op_retenciones, y redirige a la lista.
 * Compatible PHP 5.6
 */
...
// Serialize pagado details into recibe_cedula JSON string for 100% database persistence
$pagado_array = array(
    'banco'  => $pagado_banco,
...
/* Beneficiario en BD (Para listados y reportes) */
$rif_beneficiario   = 'CONTRALORÍA MUNICIPAL / BANAVIH';
...
$ret_tasa     = isset($_POST['ret_tasa'])     ? $_POST['ret_tasa']     : array(); // Will be 0
...
if ($op_id_input > 0) {
    ...
    if (!$stmt) {
        ...
        die('Error preparando UPDATE OP BANAVIH: ' . mysqli_error($conn));
    }
    ...
    mysqli_query($conn, 'SET FOREIGN_KEY_CHECKS=1');
    if (!$ok) { die('Error ejecutando UPDATE OP BANAVIH: ' . mysqli_stmt_error($stmt)); }
    ...
    if (!$stmt) {
        mysqli_query($conn, 'SET FOREIGN_KEY_CHECKS=1');
        die('Error preparando INSERT OP BANAVIH: ' . mysqli_error($conn));
    }
    mysqli_stmt_bind_param(
...
;
    ...
    if (!$ok) {
        ...
        mysqli_query($conn, 'SET FOREIGN_KEY_CHECKS=1');
        die('Error ejecutando INSERT OP BANAVIH: ' . $stmt_err);
    }
    $op_id = mysqli_insert_id($conn);
    mysqli_query($conn, 'SET FOREIGN_KEY_CHECKS=1');
    if (!$op_id) { die('Error: insert_id=0 OP BANAVIH.'); }
}
