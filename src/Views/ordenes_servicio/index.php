...
if (isset($_GET['export']) && $_GET['export'] === 'excel') {
    ...
    $filename = ($tab === 'papelera' ? 'papelera_ordenes_servicio_' : 'listado_ordenes_servicio_') . date('Y-m-d') . '.csv';
    ...
    if ($tab === 'papelera') {
        fputcsv($output, array('N° OS', 'Fecha', 'Proveedor', 'Monto Total Bs.', 'Movido a', 'Expira en (días)', 'Status'), ';');
        $where_q = $q_safe ? "AND (os.numero_os LIKE '$q_safe' OR p.razon_social LIKE '$q_safe' OR os.status LIKE '$q_safe')" : '';
        $sql = "SELECT os.numero_os, os.fecha, COALESCE(p.razon_social, '— Sin proveedor —') AS proveedor, os.monto_total, os.deleted_at, os.status
                FROM ordenes_servicio os
                LEFT JOIN proveedores p ON p.id = os.proveedor_id
                WHERE os.deleted_at IS NOT NULL $where_q
                ORDER BY os.deleted_at DESC";
        ...
        fputcsv($output, array('N° OS', 'Fecha', 'Proveedor', 'Monto Total Bs.', 'Status'), ';');
        ...
        if (!$ver_todos && preg_match('/^\d{4}-(0[1-9]|1[0-2])$/', $mes_filtro)) {
            $where_mes = "AND (os.fecha BETWEEN '{$mes_filtro}-01' AND LAST_DAY('{$mes_filtro}-01'))";
        }
        $where_q = $q_safe ? "AND (os.numero_os LIKE '$q_safe' OR p.razon_social LIKE '$q_safe' OR os.status LIKE '$q_safe')" : '';
        $sql = "SELECT os.numero_os, os.fecha, COALESCE(p.razon_social, '— Sin proveedor —') AS proveedor, os.monto_total, os.status
                FROM ordenes_servicio os
                LEFT JOIN proveedores p ON p.id = os.proveedor_id
                WHERE os.deleted_at IS NULL $where_mes $where_q
                ORDER BY os.id DESC";
        $res = mysqli_query($conn, $sql);
        if ($res) {
            while ($row = mysqli_fetch_assoc($res)) {
...
$check_col = mysqli_query($conn, "SHOW COLUMNS FROM `ordenes_servicio` LIKE 'deleted_at'");
if (mysqli_num_rows($check_col) == 0) {
    mysqli_query($conn, "ALTER TABLE ordenes_servicio ADD COLUMN deleted_at DATETIME NULL");
}

/* ── Purga automática al cargar la página (OS en papelera > 14 días) ── */
...
while ($vr = mysqli_fetch_assoc($res_viejas)) {
    $vid = (int) $vr['id'];
    mysqli_query($conn, "DELETE FROM os_partidas  WHERE os_id = $vid");
    mysqli_query($conn, "DELETE FROM os_renglones WHERE os_id = $vid");
    mysqli_query($conn, "DELETE FROM ordenes_servicio WHERE id = $vid");
}
...
$r = mysqli_query(
    $conn,
    "SELECT COALESCE(SUM(monto_total), 0) AS monto FROM ordenes_servicio
     WHERE fecha BETWEEN '$mes_inicio' AND '$mes_fin'
       AND deleted_at IS NULL"
);
$row = mysqli_fetch_assoc($r);
...
$where_q = $q_safe ? "AND (os.numero_os LIKE '$q_safe' OR p.razon_social LIKE '$q_safe')" : '';
$where_fecha = $ver_todos ? '' : "AND (os.fecha BETWEEN '$mes_inicio' AND '$mes_fin')";
$ordenes = mysqli_query(
    $conn,
    "SELECT os.id, os.numero_os, os.fecha, os.monto_total, os.status,
            COALESCE(p.razon_social, '— Sin proveedor —') AS proveedor
     FROM ordenes_servicio os
     LEFT JOIN proveedores p ON p.id = os.proveedor_id
     WHERE os.deleted_at IS NULL $where_fecha $where_q
     ORDER BY os.id DESC"
...
$papelera = mysqli_query(
    $conn,
    "SELECT os.id, os.numero_os, os.fecha, os.monto_total, os.status, os.deleted_at,
            COALESCE(p.razon_social, '— Sin proveedor —') AS proveedor
     FROM ordenes_servicio os
     LEFT JOIN proveedores p ON p.id = os.proveedor_id
...
<head>
    ...
    <meta name="description"
        content="Listado de Órdenes de Servicio - Contraloría del Municipio Simón Rodríguez, Estado Anzoátegui">
    <link
        href="/sistema/assets/fonts/fonts.css"
        rel="stylesheet">
    <link rel="icon" type="image/png" href="/sistema/assets/img/logo.png">
    <title>Órdenes de Servicio – Contraloría MSR</title>
    <style>
...
<body>
    ...
    <div class="layout">
            ...
            <div class="container">
                ...
                <div class="doc-header-institutional">
                    ...
                    <div class="header">
                        ...
                        <h2 id="print-report-title"><?php echo ($tab_activa === 'papelera') ? 'Listado de Órdenes de Servicio (Papelera)' : 'Listado de Órdenes de Servicio'; ?></h2>
                    </div>
                ...
                <div class="top-bar">
                    ...
                    <?php if ($es_admin): ?>
                        <a href="nueva_os.php" class="btn-nueva">+ NUEVA ORDEN DE SERVICIO</a>
                    <?php endif; ?>
                </div>
                ...
                <?php if (isset($_GET['ok'])): ?>
                    <div class="alert-success">
                        ...
                        <div><strong>¡ORDEN DE SERVICIO GUARDADA CORRECTAMENTE!</strong></div>
                    </div>
                <?php endif; ?>
                ...
                <?php if (isset($_GET['msg'])): ?>
                    <?php if ($_GET['msg'] === 'movido'): ?>
                        <div class="alert-success" style="background:#fff3e0; color:#e65100; border-color:#ffe0b2;">
                            <div>🗑️ La orden de servicio ha sido movida a la <strong>Papelera</strong>. Permanecerá allí
                                durante 14 días.</div>
                        </div>
                    <?php elseif ($_GET['msg'] === 'restaurado'): ?>
                        <div class="alert-success">
                            <div>↩ La orden de servicio ha sido <strong>restaurada</strong> exitosamente a la lista activa.
                            </div>
                        </div>
                    <?php elseif ($_GET['msg'] === 'eliminado'): ?>
                        <div class="alert-success" style="background:#ffebee; color:#c62828; border-color:#ef9a9a;">
                            <div>🗑️ La orden de servicio ha sido <strong>eliminada permanentemente</strong> de la base de
                                datos.</div>
                        </div>
                    <?php endif; ?>
                <?php endif; ?>
                ...
                <div class="tabs-bar no-print">
                    ...
                    <div style="display:flex; align-items:center; gap:10px; flex-wrap:wrap;">
                        <input type="text" id="busqueda-tabla" class="search-input"
                            placeholder="🔍 Buscar por N° orden, proveedor, fecha, status..."
                ...
                <div id="tab-activas" class="tab-panel <?php echo ($tab_activa === 'activas') ? 'activa' : ''; ?>">
                    <div class="section-title"><?php echo $ver_todos ? 'Todas las Órdenes de Servicio Activas' : 'Órdenes de Servicio Activas — ' . $nombre_mes . ' ' . $anio_sel; ?></div>

                    <?php if (!$ordenes || mysqli_num_rows($ordenes) === 0): ?>
                        <div class="sin-datos">No hay órdenes de servicio registradas <?php echo $ver_todos ? 'activas' : 'en ' . $nombre_mes . ' ' . $anio_sel; ?>.</div>
                    <?php else: ?>
                        <table id="tabla-ordenes">
                            ...
                            <tbody>
                                <?php while ($os = mysqli_fetch_assoc($ordenes)): ?>
                                    <?php
                                    $raw_status = isset($os['status']) ? $os['status'] : 'pendiente';
                                    ...
                                    $fecha_fmt = '';
                                    if (isset($os['fecha']) && $os['fecha'] != '') {
                                        $ts = strtotime($os['fecha']);
                                        $fecha_fmt = $ts ? date('d/m/Y', $ts) : htmlspecialchars($os['fecha']);
                                    }
                                    $num_orden = isset($os['numero_os']) ? $os['numero_os'] : $os['id'];
                                    ?>
                                    <tr>
                                        <td style="text-align:center;"><?php echo htmlspecialchars($num_orden); ?></td>
                                        <td style="text-align:center;"><?php echo $fecha_fmt; ?></td>
                                        <td><?php echo htmlspecialchars(isset($os['proveedor']) ? $os['proveedor'] : ''); ?>
                                        </td>
                                        <td style="text-align:right;">
                                            Bs.
                                            <?php echo number_format(isset($os['monto_total']) ? $os['monto_total'] : 0, 2, '.', ','); ?>
                                        </td>
                                        <td style="text-align:center;">
                                            <?php if ($es_admin || $es_supervisor): ?>
                                                <select class="badge <?php echo $badge_cls; ?>"
                                                    onchange="cambiarStatusRapido('ordenes_servicio', <?php echo (int) $os['id']; ?>, this.value, this)"
                                        ...
                                        <td style="text-align:center; white-space:nowrap;">
                                            <a href="ver_os.php?id=<?php echo (int) $os['id']; ?>" class="btn-ver">Ver</a>
                                            <?php if ($es_admin && $status_val === 'pendiente'): ?>
                                                <a href="nueva_os.php?id=<?php echo (int) $os['id']; ?>" class="btn-editar"
                                                    title="Editar esta orden de servicio">✏️ Editar</a>
                                            <?php endif; ?>
                                            <?php if ($es_admin): ?>
                                                <a href="../ordenes_pago/nueva_op.php?os_id=<?php echo (int) $os['id']; ?>"
                                                    class="btn-crear-op" title="Crear Orden de Pago basada en esta OS">💳 Crear
                                                    OP</a>
                                            <?php endif; ?>
                                            <?php if ($es_admin && $status_val === 'pendiente'): ?>
                                                <a href="../../controllers/ordenes_servicio/papelera_os.php?id=<?php echo (int) $os['id']; ?>"
                                                    class="btn-anular"
                ...
                <div id="tab-papelera" class="tab-panel <?php echo ($tab_activa === 'papelera') ? 'activa' : ''; ?>">
                    <div class="papelera-notice">
                        🕒 Las órdenes de servicio en la papelera se eliminan definitivamente de forma automática
                        después de <strong>14 días</strong>.
                        ...
                        <table id="tabla-papelera">
                            ...
                            <tbody>
                                <?php while ($pt = mysqli_fetch_assoc($papelera)): ?>
                                    <?php
                                    $num_pt = isset($pt['numero_os']) ? $pt['numero_os'] : $pt['id'];
                                    ...
                                    if (isset($pt['fecha']) && $pt['fecha'] != '') {
                                        ...
                                        $fecha_os_fmt = $ts2 ? date('d/m/Y', $ts2) : htmlspecialchars($pt['fecha']);
                                    }
                                    ?>
                                    <tr>
                                        <td style="text-align:center;"><?php echo htmlspecialchars($num_pt); ?></td>
                                        <td style="text-align:center;"><?php echo $fecha_os_fmt; ?></td>
                                        <td><?php echo htmlspecialchars(isset($pt['proveedor']) ? $pt['proveedor'] : ''); ?>
                                        </td>
                                        <td style="text-align:right;">Bs.
                                            <?php echo number_format(isset($pt['monto_total']) ? $pt['monto_total'] : 0, 2, '.', ','); ?>
                                        </td>
                                        ...
                                        <td style="text-align:center; white-space:nowrap;">
                                            <?php if ($es_admin): ?>
                                                <a href="../../controllers/ordenes_servicio/restaurar_os.php?id=<?php echo (int) $pt['id']; ?>"
                                                    class="btn-restaurar"
                                                    onclick="return confirm('¿Restaurar la orden N° <?php echo htmlspecialchars($num_pt); ?>?');">
                                                    ↩ Restaurar
                                                </a>
                                                <a href="../../controllers/ordenes_servicio/eliminar_definitivo_os.php?id=<?php echo (int) $pt['id']; ?>"
        ...
        function cambiarTab(tab) {
            ...
            if (titleEl) {
                titleEl.innerText = (tab === 'papelera') ? 'Listado de Órdenes de Servicio (Papelera)' : 'Listado de Órdenes de Servicio';
            }
        }
