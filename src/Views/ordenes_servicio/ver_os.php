...
// Obtener datos de la OS + Proveedor
$res = mysqli_query($conn,
    "SELECT os.*, p.rif, p.razon_social, p.direccion AS prov_direccion, p.telefono AS prov_telefono,
            uc.nombre AS creado_por_nombre, uu.nombre AS actualizado_por_nombre
     FROM ordenes_servicio os
     LEFT JOIN proveedores p ON p.id = os.proveedor_id
     LEFT JOIN usuarios uc ON uc.nombre = os.created_by OR uc.usuario = os.created_by
     LEFT JOIN usuarios uu ON uu.nombre = os.updated_by OR uu.usuario = os.updated_by
     WHERE os.id = $id"
);
...
// Obtener renglones
$res_renglones = mysqli_query($conn, "SELECT * FROM os_renglones WHERE os_id = $id ORDER BY id ASC");
$renglones = array();
while ($r = mysqli_fetch_assoc($res_renglones)) {
    $renglones[] = $r;
}

// Obtener partidas
$res_partidas = mysqli_query($conn, "SELECT * FROM os_partidas WHERE os_id = $id ORDER BY id ASC");
$partidas = array();
while ($p = mysqli_fetch_assoc($res_partidas)) {
    $partidas[] = $p;
}

$status = isset($os['status']) ? $os['status'] : 'pendiente';
$fecha_fmt = date('d/m/Y', strtotime($os['fecha']));
...
<head>
    <meta charset="UTF-8">
    <meta name="description" content="Ver Orden de Servicio N° <?= htmlspecialchars($os['numero_os']) ?>">
    <title>Orden de Servicio N° <?= htmlspecialchars($os['numero_os']) ?> — Contraloría MSR</title>
    ...
    /* Premium typography: IBM Plex Sans headings, Inter body */
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .os-document { border: 2px solid #000; padding: 12px; }
        ...
        .title-cell { background: #1565c0; color: white; display: flex; align-items: center; justify-content: center; padding: 6px; }
        .title-cell h2 { font-size: 16px; letter-spacing: 1px; }
        ...
        .section-title { background: #1565c0; color: white; font-weight: bold; font-size: 11px; text-align: center; padding: 4px; text-transform: uppercase; margin-bottom: 0; border: 1px solid #000; border-bottom: none; }
        ...
        @media print {
            .site-header, .sidebar, .btns, .no-print { display: none !important; }
            body { background: white; font-size: 10pt; }
            .container { border: none; box-shadow: none; padding: 0; width: 100%; max-width: none; }
            .main-content { padding: 0; }
            .os-document { border: 2px solid #000; }
        }
    </style>
</head>
<body><div class="layout">
    <?php $active = 'os-lista'; require_once __DIR__ . '/../../../includes/sidebar.php'; ?>
        ...
        <div class="container">
            ...
            <div class="os-document">
                ...
                <div class="title-row">
                    <div class="title-cell">
                        <h2>ORDEN DE SERVICIOS</h2>
                    </div>
                    <div class="info-cell">
                        <div>N° Orden</div>
                        <div>Fecha</div>
                        <div><?= htmlspecialchars($os['numero_os']) ?></div>
                        <div><?= $fecha_fmt ?></div>
                    </div>
                </div>
                ...
                <div class="prov-block">
                    <div class="prov-left">
                        <div class="prov-row"><label>Proveedor:</label> <?= htmlspecialchars($os['rif']) ?> — <?= htmlspecialchars($os['razon_social']) ?></div>
                        <div class="prov-row"><label>Dirección:</label> <?= htmlspecialchars(strtoupper($os['prov_direccion'])) ?></div>
                        <div class="prov-row"><label>Teléfono:</label> <?= htmlspecialchars($os['prov_telefono']) ?></div>
                        <div class="prov-row"><label>Lugar Entrega:</label> <?= htmlspecialchars(strtoupper($os['lugar_entrega'])) ?></div>
                    </div>
                ...
                <div class="section-title">RENGLONES DE LA ORDEN DE SERVICIOS</div>
                <table>
                    <thead>
                        <tr>
                ...
                <div class="totales-wrap">
                    <div class="totales-box">
                        <div class="tot-row"><span>Base Imponible:</span><span>Bs. <?= number_format($os['base_imponible'], 2, '.', ',') ?></span></div>
                        <div class="tot-row"><span>SAT 0,1%:</span><span>Bs. <?= number_format($os['sat_monto'], 2, '.', ',') ?></span></div>
                        <div class="tot-row"><span>Sub-Total:</span><span>Bs. <?= number_format($os['base_imponible'] + $os['sat_monto'], 2, '.', ',') ?></span></div>
                        <div class="tot-row"><span>IVA 16%:</span><span>Bs. <?= number_format($os['iva_monto'], 2, '.', ',') ?></span></div>
                        <div class="tot-row total"><span>Total General:</span><span>Bs. <?= number_format($os['monto_total'], 2, '.', ',') ?></span></div>
                    </div>
                </div>

                <div class="monto-letras">
                    Monto total en letra: <strong><?= htmlspecialchars($os['monto_letras']) ?></strong>
                </div>
                ...
                <p class="condicion">
                    El organismo se reserva el derecho de anular unilateralmente la presente orden de servicio sin indemnización de conformidad con lo dispuesto en la ley que rige la materia.
                </p>
                ...
                <div style="margin-top: 15px; padding: 8px 12px; background: #f8fafc; border: 1px solid #cbd5e1; border-radius: 4px; font-size: 9.5px; color: #475569; display: flex; justify-content: space-between; align-items: center;" class="no-print">
                    <div>
                        <strong>👤 Creado por:</strong> <?= htmlspecialchars(!empty($os['creado_por_nombre']) ? $os['creado_por_nombre'] : (!empty($os['created_by']) ? $os['created_by'] : 'Sistema')) ?>
                        <?php if (!empty($os['created_at'])): ?>
                            <span style="color:#64748b;">(<?= date('d/m/Y h:i A', strtotime($os['created_at'])) ?>)</span>
                        <?php endif; ?>
                    </div>
                    <?php if (!empty($os['updated_by'])): ?>
                    <div>
                        <strong>✏️ Última modificación:</strong> <?= htmlspecialchars(!empty($os['actualizado_por_nombre']) ? $os['actualizado_por_nombre'] : (!empty($os['updated_by']) ? $os['updated_by'] : 'Sistema')) ?>
                        <?php if (!empty($os['updated_at'])): ?>
                            <span style="color:#64748b;">(<?= date('d/m/Y h:i A', strtotime($os['updated_at'])) ?>)</span>
                        <?php endif; ?>
                    </div>
                    <?php endif; ?>
                </div>
            ...
            <div class="status-change-panel no-print" style="margin-top: 18px; padding: 12px 18px; background: #ffffff; border: 1px solid #cbd5e1; border-radius: 8px; box-shadow: 0 2px 6px rgba(0,0,0,0.05); display: flex; align-items: center; justify-content: space-between; gap: 12px; flex-wrap: wrap;">
                ...
                <div style="display: flex; align-items: center; gap: 8px;">
                    ...
                    <select id="select-status-detail" onchange="cambiarStatusDetalle('ordenes_servicio', <?= $os['id'] ?>, this.value)" style="padding: 6px 12px; font-size: 11.5px; border: 1.5px solid #cbd5e1; border-radius: 6px; font-weight: 600; background: #f8fafc; cursor: pointer; outline: none;">
            ...
            <div class="btns no-print">
                ...
                <?php if ($status === 'pendiente'): ?>
                    <a href="nueva_os.php?id=<?= $os['id'] ?>" class="btn-editar">
                        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
                    ...
                    <a href="../../controllers/ordenes_servicio/papelera_os.php?id=<?= $os['id'] ?>" class="btn-anular" onclick="return confirm('¿Desea anular esta orden de servicio?')">
                        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
