...
if (isset($_GET['export']) && $_GET['export'] === 'excel') {
    ...
    $filename = 'listado_proveedores_' . date('Y-m-d') . '.csv';
    ...
    fputcsv($output, array('ID', 'RIF', 'Razón Social', 'Dirección', 'Teléfono', 'Email', 'Fecha de Registro'), ';');
    $where_q = $q_safe ? "WHERE (rif LIKE '$q_safe' OR razon_social LIKE '$q_safe' OR direccion LIKE '$q_safe' OR telefono LIKE '$q_safe' OR email LIKE '$q_safe')" : '';
    $sql = "SELECT id, rif, razon_social, direccion, telefono, email, created_at FROM proveedores $where_q ORDER BY id DESC";
    $res = mysqli_query($conn, $sql);
    if ($res) {
        while ($row = mysqli_fetch_assoc($res)) {
            $ts = isset($row['created_at']) && $row['created_at'] ? strtotime($row['created_at']) : false;
            $fecha_fmt = $ts ? date('d/m/Y', $ts) : '';
...
$query = "SELECT id, rif, razon_social, direccion, telefono, email, created_at FROM proveedores ORDER BY id DESC";
...
<head>
    ...
    <meta name="description"
        content="Listado de proveedores del sistema de gestión - Contraloría del Municipio Simón Rodríguez">
    <link
        href="/sistema/assets/fonts/fonts.css"
        rel="stylesheet">
    <link rel="icon" type="image/png" href="/sistema/assets/img/logo.png">
    <title>Listado de Proveedores – Contraloría MSR</title>
    <style>
    ...
    <script>
        ...
        function filtrarTablaProveedores(valor) {
            var term = (valor || '').toLowerCase().trim();
            var table = document.getElementById('tabla-proveedores');
            if (!table) return;
        ...
        <main class="main-content">
            ...
            <div class="doc-header-institutional">
                ...
                <div class="doc-title-center">
                    ...
                    <h2 id="print-report-title">Listado de Proveedores</h2>
                </div>
            ...
            <div class="action-bar no-print">
                ...
                <?php if ($es_admin): ?>
                    <a href="nuevo.php" class="btn-new-provider">
            ...
            <?php if (isset($_GET['error']) && $_GET['error'] == 'tiene_ordenes'): ?>
                <div class="alert alert-danger">
                    Error: No se puede eliminar el proveedor porque tiene órdenes de compra o pago asociadas.
                </div>
            <?php endif; ?>

            <?php if (isset($_GET['error']) && $_GET['error'] == 'db_error'): ?>
                <div class="alert alert-danger">
                    Error: Ocurrió un error al intentar eliminar el proveedor en la base de datos.
                </div>
            <?php endif; ?>
            ...
            <div class="panel">
                <div class="panel-head">
                    ...
                    <div style="display:flex; align-items:center; gap:10px; flex-wrap:wrap;">
                        <input type="text" id="busqueda-tabla" class="search-input" placeholder="Buscar por RIF, nombre, teléfono, email..."
                            onkeyup="filtrarTablaProveedores(this.value)">
                        ...
                        <table id="tabla-proveedores">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 12%; text-align: center;">RIF</th>
                                    <th style="width: 25%;"><span class="th-icon"><svg width="12" height="12"
                                                ...
                                                <path d="M23 21v-2a4 4 0 0 0-3-3.87" />
                                                <path d="M16 3.13a4 4 0 0 1 0 7.75" />
                                            </svg>Razón Social</span></th>
                                    <th style="width: 25%;"><span class="th-icon"><svg width="12" height="12"
                                                viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                                stroke-linecap="round" stroke-linejoin="round">
                                                <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" />
                                                <circle cx="12" cy="10" r="3" />
                                            </svg>Dirección</span></th>
                                    <th style="width: 10%; text-align: center;"><span class="th-icon"><svg width="12"
                                                height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                                <path
                                                    d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z" />
                                            </svg>Teléfono</span></th>
                                    <th style="width: 13%;"><span class="th-icon"><svg width="12" height="12"
                                                viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                                stroke-linecap="round" stroke-linejoin="round">
                                                <path
                            ...
                            <tbody>
                                <?php while ($row = mysqli_fetch_assoc($result)): ?>
                                    ...
                                    ?>
                                    <tr>
                                        <td style="text-align: center;"><?php echo $prov_id; ?></td>
                                        <td style="text-align: center; font-weight: bold;">
                                            <?php echo htmlspecialchars(isset($row['rif']) ? $row['rif'] : ''); ?>
                                        </td>
                                        <td><?php echo htmlspecialchars(isset($row['razon_social']) ? $row['razon_social'] : ''); ?>
                                        </td>
                                        <td><?php echo htmlspecialchars(isset($row['direccion']) && $row['direccion'] !== '' ? $row['direccion'] : '—'); ?>
                                        </td>
                                        <td style="text-align: center;">
                                            <?php echo htmlspecialchars(isset($row['telefono']) && $row['telefono'] !== '' ? $row['telefono'] : '—'); ?>
                                        </td>
                                        <td><?php echo htmlspecialchars(isset($row['email']) && $row['email'] !== '' ? $row['email'] : '—'); ?>
                                        </td>
                                        <td style="text-align: center;"><?php echo $fecha_fmt; ?></td>
                                        <td style="text-align: center; white-space: nowrap;">
                                            <a href="/sistema/src/Views/proveedores/editar.php?id=<?php echo $prov_id; ?>"
                                            ...
                                            <?php if ($es_admin): ?>
                                                <a href="javascript:void(0);"
                                                    onclick="confirmarEliminacion('../../controllers/proveedores/eliminar.php?id=<?php echo $prov_id; ?>')"
